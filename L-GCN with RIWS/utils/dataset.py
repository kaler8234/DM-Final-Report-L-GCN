...
from utils.misc import TextColors, l2norm, read_probs, read_meta, read_label_npy
...
class BasicDataset():
    def __init__(self,
        ...
        self.prefix = prefix
        self.label_path = os.path.join(prefix, 'labels', name + '.npy')
        if os.path.isfile(self.label_path):
            # self.lb2idxs, self.idx2lb = read_meta(self.label_path,
            #                                       verbose=verbose)
            self.lb2idxs, self.idx2lb, _ = read_label_npy(self.label_path,
                                                  verbose=verbose)
        ...
        self.feat_path = os.path.join(prefix, 'features', name + '.npy')
