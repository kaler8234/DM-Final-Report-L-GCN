...
do
  ...
  do
    ...
    python -m torch.distributed.launch --nproc_per_node=1 --master_port 18115 train_dist.py \
           ...
           --logs-dir ${cur_save_dir} --print_freq 20 --workers 0 \
           --rand_val 0 --rand_twice 0 --resample 0 --is_density 0 --clique_size 0 --batch_size 16 \
           --lr 0.01 --epochs 1 --features 256 --k-at-hop 80 10  --active_connection 10 \
    ...
    do
      ...
      python -u -m torch.distributed.launch --nproc_per_node=1 --master_port 18125 test_dist.py \
             --val_feat_path /workspace/data/20200811_clean/random_celeb/${cur_dir}/test_feat_celeb_random.lmdb \
             --val_knn_graph_path /workspace/data/20200811_clean/random_celeb/${cur_dir}/test_feat_celeb_random/knn_index_200.npy \
             --val_label_path /workspace/data/20200811_clean/random_celeb/${cur_dir}/test_label_celeb_random.npy \
             --checkpoint $modelfile --batch_size 16 --max_size 300 \
             --features 256 --workers 0 --print_freq 20  --k-at-hop 79 5 \
             --active_connection 5 | tee -a $cur_save_dir/eval.log
    done
  done
done