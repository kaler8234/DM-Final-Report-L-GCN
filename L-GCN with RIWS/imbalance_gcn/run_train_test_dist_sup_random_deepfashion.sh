...
do
  ...
  do
    ...
    python -m torch.distributed.launch --nproc_per_node=1 --master_port 21001 train_dist.py \
           ...
           --logs-dir ${cur_save_dir} --print_freq 20 --workers 1 \
           --rand_val 0 --rand_twice 0 --is_density 0 --clique_size 0 --batch_size 16 \
           --lr 0.01 --epochs 10 --features 256 --k-at-hop 5 5  --active_connection 5 \
           --model_drop gcn --block_size 3 --drop_prob 0.7 \
           --loss FL1 --ratio 0.5 --gamma 2 --alpha 0.75 \
           --mode sup | tee ${cur_save_dir}/log.out

    echo now is test stage
    for idx in $(seq 10 10)
    do
      echo $idx
      ...
      #        --active_connection 5 | tee -a $cur_save_dir/eval.log
        ...
        python -u -m torch.distributed.launch --nproc_per_node=1 --master_port 21111 test_dist.py \
               --val_feat_path /workspace/data/20200811_clean/random_deepfashion/${cur_dir}/test_feat_deepfashion_random.lmdb \
               --val_knn_graph_path /workspace/data/20200811_clean/random_deepfashion/${cur_dir}/test_feat_deepfashion_random/knn_index_20.npy \
               --val_label_path /workspace/data/20200811_clean/random_deepfashion/${cur_dir}/test_label_deepfashion_random.npy \
               --checkpoint $modelfile --batch_size 16 --max_size 50 --prop_step 0.5 \
               --features 256 --workers 1 --print_freq 20  --k-at-hop 5 5 \
               --active_connection 5 | tee -a $cur_save_dir/eval.log
    done
