...
import model
from feeder.feeder_test import Feeder
...
from utils.graph import graph_propagation, graph_propagation_soft, graph_propagation_naive, graph_propagation_thresh
...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, metavar='PATH', default='./')
...
def main_worker(gpu, ngpus_per_node, world_size, args):
    ...
    if process_rank == 0:
        ...
        prefix = os.path.join(prefix, args.save_dir)
        ...
        pr_auc, pr_auc0 = cal_edge_pre_rec(gather_edges, gather_scores, labels)
        print('------------------------------------')
        print("pr_auc: ", pr_auc)
        print("pr_auc0: ",pr_auc0)
        print('------------------------------------')

        # clusters = graph_propagation(gather_edges, gather_scores, max_sz=900, step=0.6, pool='avg')
        clusters_merge = graph_propagation(gather_edges, gather_scores, max_sz=args.max_size, step=args.prop_step,
                                     pool=args.pool)
        # clusters_thresh = graph_propagation_thresh(gather_edges, gather_scores, labels)
        # import sys
        # sys.setrecursionlimit(100000)
        # clusters_np = np.array(clusters)
        # np.savetxt('clusters_np.txt', clusters_np)

        final_pred = clusters2labels(clusters_merge, len(valset))
        # final_pred = graph_propagation_threh(gather_edges, gather_scores, labels)
...
def validate(loader, net, crit, args, process_rank):
    ...
    for i, ((feat, adj, cid, h1id, node_list), gtmat) in enumerate(loader):
        ...
        feat, adj, cid, h1id, gtmat = map(lambda x: x.cuda(args.gpu, non_blocking=True), 
    ...
    if process_rank == 0:
        print('Mean = ', np.mean(np.array(loss_list)))
        print('Median = ', np.median(np.array(loss_list)))
        print('Var = ', np.var(np.array(loss_list)))
...
def cal_edge_pre_rec(edges,scores,labels):
    # scores=np.ones(46137264)
    edges = np.sort(edges, axis=1)
    score_dict = {}  # score lookup table
    for i, e in enumerate(edges):
        # if score_dict.has_key((e[0],e[1])):
        if (e[0], e[1]) in score_dict:
            score_dict[e[0], e[1]] = 0.5 * (score_dict[e[0], e[1]] + scores[i])
        else:
    ...
    for k in score_dict.keys():
        if labels[k[0]] == labels[k[1]]:
            ...
            gt.append(0)
    unique_scores = list(score_dict.values())
    precision, recall, threshold = metrics.precision_recall_curve(gt, unique_scores)
    pr_auc = metrics.auc(recall, precision)
    pr_auc0 = metrics.average_precision_score(gt, unique_scores)

    return (pr_auc,pr_auc0)
