...
def graph_propagation_naive(edges, score, th):
    ...
    mapping = -1 * np.ones((nodes.max()+1), dtype=np.int64)
...
def graph_propagation(edges, score, max_sz, step=0.1, beg_th=0.9, pool=None):
    ...
    if pool is None:
        ...
        for i, e in enumerate(edges):
            if score_dict.__contains__((e[0], e[1])):
                score_dict[e[0], e[1]] = 0.5 * \
                    (score_dict[e[0], e[1]] + score[i])
            else:
                score_dict[e[0], e[1]] = score[i]
...
def graph_propagation_soft(edges, score, max_sz, step=0.1, **kwargs):
    ...
    label, label_fusion = diffusion(
        prop_vertex, label_dict, score_dict, **kwargs)
...
def diffusion(vertex, label, score_dict, max_depth=5, weight_decay=0.6, normalize=True):
    ...
    for root in vertex:
        if prog % prog_step == 0:
            print("progress: {} / {}, elapsed time: {}".format(prog,
                  len(vertex), time.time() - start))
        prog += 1
        # queue = {[root, 0, 1.0]}
        ...
        while queue:
            ...
            for n in neighbors:
                if n.name not in visited:
                    sub_value = score_dict[tuple(
                        sorted([curr.node.name, n.name]))] * weight_decay * curr.value
            ...
            for si in sortidx:
                queue.add(
                    BFSNode(tmp_neighbor[si], curr.depth+1, tmp_value[si]))
    if normalize:
